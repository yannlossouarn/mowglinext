 ...
 * to wind up to reach it — which is what produced the stick-slip lurch and the
 * yaw/position corruption seen during low-speed autonomous pivots. 0 disables
 * it (open-loop mapping unchanged); the loop gains/limits still apply on top.
 *
 * wheel_pi_enabled selects the per-wheel loop at runtime (1 = closed-loop PI
 * with the gains above, 0 = open-loop feedforward only), replacing the old
