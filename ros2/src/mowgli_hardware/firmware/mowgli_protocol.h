 ...
 *  copies of the detector's compile-time magnitude thresholds. */
...
#define HL_MODE_MANUAL_MOWING 4u /**< Manual teleop with blade */
   ...
   *
   * hold_enabled + hold_kp drive the standstill position hold: while actively
   * controlled and commanded to ~0 velocity the firmware latches the encoder
   * position and applies sign(err)*deadband_pwm + hold_kp*err_ticks to resist
   * the post-pivot creep, releasing (coasting) only when really idle (IDLE mode
   * or emergency).
   *
...
#define DRIVE_SLIP_FLAG_JAM \
   ...
   *
   * Runtime-tunable copies of the discrepancy-detector magnitude thresholds.
   * Firmware validates/clamps every field; debounce counts stay compile-time.
   *
...
#pragma pack(pop)
  ...
  _Static_assert(sizeof(pkt_set_detector_params_t) == 35u,
                 "pkt_set_detector_params_t layout unexpected");
#endif
