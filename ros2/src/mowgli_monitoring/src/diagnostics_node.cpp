...
void DiagnosticsNode::declare_parameters()
{
  ...
  expect_lidar_ = declare_parameter<bool>("expect_lidar", true);
...
diagnostic_msgs::msg::DiagnosticStatus DiagnosticsNode::check_lidar(const rclcpp::Time& now) const
{
  ...
  // GPS-only robots run without a LiDAR by design — a missing scan is then the
  // expected steady state, not a fault. Gated on `expect_lidar` (wired from the
  // launch's resolved use_lidar / mowgli_robot.yaml.lidar_enabled).
  if (!expect_lidar_)
  {
    status.level = DiagLevel::OK;
    status.message = "LiDAR not configured";
    return status;
  }
